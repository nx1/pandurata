...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  for (ir=0;ir<=Nr;ir++) {
    ...
    if (rr[ir] < Rhor) {
	    irstart++;
	    printf("irstart %d %12.5e %12.5e %d \n", ir, rr[ir], Rhor, irstart);
    }
  }
  dph = (pp[Nph]-pp[0])/Nph;
  dth = (tt[Nth]-tt[0])/Nth;
 ...
 printf("Back to pandurata from surface_tetrads \n");
	...
	for (ip=0;ip<=N;ip++) {
	  ...
	  while ((yn[1]>1.02*Rhor)&&(yn[1]<Rshell*0.99999)&&(steps<1000)&&(A_fact>0)) {
		...
		y2[j]=y_ck[j];
	      ...
	      //printf("y_ck %12.5e %12.5e %12.5e \n",y_ck[1],y_ck[2],y_ck[3]);
		...
		old_zone = 2;
	      ...
	      //printf("%12.5e %12.5e %12.5e %12.5e\n",
              //           yn[1],yn[3],Rout,Rhor);
	      /*printf("%12.5e %12.5e %d %12.5e %d\n",
			 y2[1],y2[3],ir_new,f_new,jph_new);*/
	      new_zone = 0;
		...
		printf("embedded %ld %d %d %10.4e %10.4e %ld %d %10.4e %10.4e\n",
              ...
              //printf("ir_new %12.5e %12.5e %12.5e %12.5e %d %d %d %d\n", y2[1], Rin, Rout, Rhor, Nr, ir_new, old_zone, new_zone);	      
		...
		/*printf("e_lfs \n");
                  printf("%12.5e %12.5e %12.5e %12.5e\n",
			 e_lfs[0][0],e_lfs[0][1],e_lfs[0][2],e_lfs[0][3]);
		  printf("%12.5e %12.5e %12.5e %12.5e\n",
			 e_lfs[1][0],e_lfs[1][1],e_lfs[1][2],e_lfs[1][3]);
		  printf("%12.5e %12.5e %12.5e %12.5e\n",
			 e_lfs[2][0],e_lfs[2][1],e_lfs[2][2],e_lfs[2][3]);
		  printf("%12.5e %12.5e %12.5e %12.5e\n",
			 e_lfs[3][0],e_lfs[3][1],e_lfs[3][2],e_lfs[3][3]);*/
		for (i=0;i<=3;i++) {
		  for (j=0;j<=3;j++) {
		...
		if (isbottom == 0) {
		  for (i=0;i<=3;i++) {
		    for (j=0;j<=3;j++) {
		      ...
		      //printf("emtop_elf_ik return %d %d %d %d %d %12.5e\n", i,j,ir_new,jph_new,indexelf(ir_new,jph_new,i,j),emtop_elf_ik[indexelf(ir_new,jph_new,i,j)]);
		    }
		    v_[i]=e_lfs[0][i];
		  }
		  yn[1]=rr[ir_new];
