...
// Calculate the disk spectrum using the Novikov-Thorne (NT) accretion disk model
		 ...
		 double nu[], double Inur[], double Ts_r[], double qnur[])
{
  ...
  Mbh = Mstar*3.*1.99e33; // 1.31937e+34
  kap_es = 0.4; // Electron scattering opacity (0.4 cm^2/g).
  f_hard = 1.8; // Hardening factor (correction to blackbody spectrum).
  a2 = aa*aa;
  ...
  Re      = 1.001*(1.+sqrt(1.-aa*aa));  // 1.44
  Lisco   = 2. / sqrt(3.) * (3.*sqrt(Risco)-2.*aa) / sqrt(Risco);
  Eisco   = (Risco*Risco-2*M*Risco+aa*sqrt(M*Risco)) / (Risco*sqrt(Risco*Risco-3*M*Risco+2*aa*sqrt(M*Risco)));
  ...
  // Move to the radial bin position outside the ISCO in the rr array.
  j=-1;
  for (i=0;i<=Nr;i++) {
    if (rr[i]<Risco) j=i;
  }
  ...
  for (i=ir_in; i<=Nr; i++) {
    ...
    // Calculate NT variables
    // https://www.its.caltech.edu/~kip/index.html/PubScans/II-48.pdf pg 422
    ...
    // integration from r to r_out to calculate NT J factor
    u_star = 1. / rr[i];
    du = u_star / Nr;
    subtot = 0.0;
    for (j=0; j<=Nr; j++) {
    ...
    // integrate from r_in to r_i to get NT Q factor
    subtot = 0.;
    for (j=ir_in; j<=i; j++) {
    ...
    flux_r[i]  = 6.e25*(Mdot_17/(Mstar*Mstar))*ntQ[i] / (pow(rr[i],3.)*ntB[i]*sqrt(ntC[i]));
    sdens_r[i] = 20. * (Mstar/Mdot_17/nt_alpha)*pow(rr[i],1.5)*pow(ntB[i],3.) * sqrt(ntC[i])*ntE[i]/(ntA[i]*ntA[i]*ntQ[i]);
    h_r[i]     = 1.e5 * Mdot_17*ntA[i]*ntA[i]*sqrt(ntC[i])*ntQ[i] / (pow(ntB[i],3.)*ntD[i]*ntE[i]);
    rhoc_r[i]  = sdens_r[i] / (2.*h_r[i]);
    Tc_r[i]    = 4.e7*pow(nt_alpha*Mstar,-0.25)*sqrt(ntB[i])*pow(ntE[i],0.25) / (pow(rr[i], 3./8.)*sqrt(ntA[i]));
